    ...
    return (
        <div className={cn("relative w-full h-full", className)}>
            <div className="absolute inset-0 bg-zinc-900 border-4 border-dashed border-zinc-700 flex flex-col items-center justify-center z-0">
                <span className="text-zinc-400 font-black text-2xl uppercase tracking-widest mb-2">Next Gen</span>
                {isBroken && (
            ...
            <div
                ...
                className={cn(
                    "relative z-10 bg-white dark:bg-zinc-800 border-4 border-black p-6 shadow-neo cursor-grab active:cursor-grabbing select-none h-full flex flex-col justify-between overflow-hidden transition-[background-color]",
            ...
            >
                <div>
                    <div className="flex justify-between items-start mb-4">
                        <h3 className="font-black text-2xl uppercase tracking-tighter leading-none text-black dark:text-white">{title}</h3>
                        <div className="w-8 h-8 bg-black dark:bg-orange-500 rounded-full flex-shrink-0" />
                    </div>
                    <p className="font-bold text-lg leading-tight text-black dark:text-zinc-300">{description}</p>
                </div>
                <div className="mt-8 border-t-2 border-black pt-2 flex justify-between text-xs font-mono uppercase text-black dark:text-zinc-400">
                    <span>REV. {figNum.toString().padStart(2, '0')}</span>
                    <span className={cn(crackLevel > 0.7 && "text-red-600 font-bold")}>
                        {crackLevel > 0 ? `${Math.round(crackLevel * 100)}% DMG` : 'VERIFIED'}
                    </span>
                </div>
